...
module.exports = kconfig = async (kill, message) => {
    ...
    try {
        ...
        const mess = {
            wait: '¡El amor entendido es! Espera un poco para que podamos hablar de nuevo, ok?',
            error: {
                St: `Lo usaste mal jaja!\nPara usar esto, envíe o etiquete una foto con este mensaje, si es un gif, use el comando ${prefix}gif.`,
				...
				Ga: 'Solo los administradores pueden usarlo, así que intenta convertirte en un jaja!',
				Gp: 'Lo siento, pero este es un comando para grupos..',
				Ac: `Solo los grupos que permiten contenido +18 pueden usar comandos como este, si usted es el propietario y desea esto, use ${prefix}nsfw enable, o usar en PV.`,
				Ba: 'Estimado administrador, si desea que use estos comandos, debe dejarme ser un admin!',
                Iv: '¿Es correcto este enlace? Me parece mal...'
            }
        }
        ...
        if (isGroupMsg && autoSticker && isMedia && isImage && !isCmd) {
            ...
            await kill.sendImageAsSticker(from, imageBase64, { author: '🎁 Abba', pack: '🔰 Abba ⚜️', keepScale: true })
        }
        ...
        if (isGroupMsg && autoSticker && isMedia && isVideo && !isCmd) {
            ...
            await kill.sendMp4AsSticker(from, videoBase64, null, { stickerMetadata: true, pack: '🔰 Abba ⚜️', author: '🎁 Abba ☆', fps: 30, startTime: '00:00:00.0', endTime : '00:00:05.0', crop: false, loop: 0 })
        }
        ...
        case 'stiker':
            if (isMedia && isImage) {
                ...
                const url = args[0]
                if (isUrl(url)) {
                    await kill.sendStickerfromUrl(from, url, { method: 'get' }, { author: '🎁 Abba ☆', pack: '🔰 Abba ⚜️', keepScale: true })
		...
		case 'ttp':
			...
			.then(res => {
				kill.sendImageAsSticker(from, res.data.result, { author: '🎁 Abba ☆', pack: '🔰 Abba ⚜️', keepScale: true })
			})
			break
        ...
        case 'gif':
            if (isMedia && type === 'video' || mimetype === 'image/gif' || isQuotedVideo || isQuotedGif) {
                ...
                try {
                    ...
                    await kill.sendMp4AsSticker(from, gifSticker, null, { stickerMetadata: true, pack: '🔰 Abba ⚜️', author: '🎁 Abba ☆', fps: 30, startTime: '00:00:00.0', endTime : '00:00:05.0', crop: false, loop: 0 })
                } catch (err) {
                    console.error(err)
                    await kill.reply(from, 'Esta pegatina tiene errores, es probable que sea su peso, el máximo es 1 MB.', id)
                }
            } else {
		...
		case 'legiao':
			if (isGroupMsg) return kill.reply(from, 'Puede ser que este grupo no permita enlaces, así que use este comando en el PV bien?', id)
			await kill.sendLinkWithAutoPreview(from, 'https://chat.whatsapp.com/LcoHswUSapp0MBk2rNjMXX', 'Qué bueno que te interesaste en Legion inmortal!\nAhí está nuestro grupo!', id)
			break
       ...
       case 'translate':
        ...
        case 'criador':
			...
			kill.reply(from, `wa.me/${config.owner.replace('@c.us', '')}\n\nSi no responde, simplemente espere, es raro que deje Internet. ~El pequeño adicto sabe~, pero si pasaba era algo importante.`, id)
            break
       ...
       case 'ins': 
        ...
        case 'welcome':
			if (!isGroupMsg) return kill.reply(from, mess.error.Gp, id)
			if (isGroupMsg && isGroupAdmins || isGroupMsg && isOwner) {
				if (args.length !== 1) return kill.reply(from, 'Olvidaste ponerlo si lo quieres activado [on], o desactivado [off].', id)
				if (args[0] == 'on') {
					welkom.push(chat.id)
					...
					fs.writeFileSync('./lib/config/welcome.json', JSON.stringify(welkom))
					kill.reply(from, '¡Comprendido! Inhabilité las opciones de Bienvenida y Adiós.', id)
				} else {
					kill.reply(from, 'Você esqueceu de colocar se quer ativado [on], ou desativado [off].', id)
				}
			} else if (isGroupMsg) {
       ...
       case 'clima':
        ...
        case 'profile':
            if (isGroupMsg) {
				if (!quotedMsg) {
					...
					await kill.sendFileFromUrl(from, pfp, 'pfo.jpg', `*Dados de su perfil..* ✨️ \n\n 🔖️ *¿Cuál es tu Usertag?? ${namae}*\n\n👑️ *Administrador? ${adm}*\n\n📵 *Bloqueado? ${bloqk}*\n\n💌️ *Frase do recado?*\n${status}\n\n️📈 *Level: ${peoLevel}*\n\n🕹️ *XP: ${peoXp} / ${ineedxp}*\n\n🌐 *Patente: ${patente}*`)
			    } else if (quotedMsg) {
        ...
        case 'menu':
            ...
            kill.reply(from, `De otros comandos tenemos...\n\n*${prefix}Admins* _es para administradores._\n\n*${prefix}Kill* _es solo para mi dueño._\n\n*${prefix}Adult* _es el menú de comandos para adultos._\n\n*${prefix}Down* _es el menú de descarga de música y video._\n\n_Si quieres ganar XP, chatea y usa el BOT._`, id)
            break
		...
		case 'help':
			...
			if (isGroupMsg) {
				await kill.sendText(ownerNumber, `⚠️ _Solicitud de soporte realizada por_ *${name}*, _a pedido de_ *${pushname}* _do número_ wa.me/${sender.id.replace('@c.us', '')}.\n\n_Motivo:_ ${body.slice(6)}`)
				await kill.sendText(ownerNumber, `${prefix}enviar -gp ${hpgp} | Responda com a solução`)
			} else {
				await kill.sendText(ownerNumber, `⚠️ _Solicitud de soporte realizada por_ *${pushname}* _do número_ wa.me/${sender.id.replace('@c.us', '')}.\n\n_Motivo:_ ${body.slice(6)}`)
        ...
        case 'give':
			...
			// Por Leonardo, #18, Mejoras mías en caso de que envíes el mensaje antes de agregar.
		case 'softban':
			if (isGroupMsg && isGroupAdmins || isGroupMsg && isOwner) {
				if (!isBotGroupAdmins) return kill.reply(from, mess.error.Ba, id)
				if (quotedMsg) {
		...
		//kill.sendText(ownerNumber, `_Hola querido dueño(a)!_\n_Recibí errores al ejecutar el comando..._\n\n*${prefix}${body.slice(1)}*\n\n_Te pido que lo corrijas para que podamos usarlo sin preocupaciones._\n_Agradecida, Íris._\n\n_Cual error?_\n\n*${err}*`)
